...
EOF
                        ...
                        then
                                ...
                                SELECT concat('"|',COLUMN_NAME,'|",')  AS column_name FROM information_schema.columns WHERE table_schema='${MySQLSCHEMA}' and table_name = lower('${range[$i]}');
EOF
                                #MySQL保留字处理

								tmp1=`cat ${logdir}/mysqlcol_${range[$i]}.log | grep "\"," | sed 's/|//g' | grep -v "||" | xargs`
                                colname=`echo ${tmp1%,*}`
                                ORASQL="select ${colname} from ${range[$i]}"
                                #存在则判断mysql中是否存在
                                tmp2=`cat ${logdir}/mysqlcol_${range[$i]}.log | grep "\"" | sed 's/|/\`/g' | sed ':a;N;s/\n/ /;t a ;' | sed s/[[:space:]]//g`
                                column=`echo ${tmp2%,*}`
                                MySQLCOL=$column
...
if [ $# == 10 ]
 then
  ...
  echo "
   ...
   MySQLUSERNAME:       MySQLUSERNAME用户名
   MySQLPASSWORD:       MySQLPASSWORD密码
   eg sh OraToMySQL.sh 10.45.15.205 1521 orcl scott abc123 10.45.15.205 3306 test root abc123"
   exit 0
fi